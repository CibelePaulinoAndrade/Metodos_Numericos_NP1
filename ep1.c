...
void metodo_newton(Polinomio pol,double limite1){
	//O método recebe o Polinômio base e o limite superior calculado na função de lagrange, depois de alocado um novo polinomio
	//ele passa os valores da derivada do polinomio base para o novo, e então apartir de uma variavel x0 que guarda
	//o limite superior, ele calcula f(x0) e f'(x0), que ficarão armazenadas nas variaveis soma e somaderivada
	//respectivamente, e depois divide a soma com a somaderivada e então subtrai com x0, que será atribuido
	//a x1.
	...
	while(!(x0-x1<0.00000001)&&interacoes<1001){		//encerra quando for menor que a oitava casa decimal ou ocorrer as 1000 interacoes
		if(interacoes!=0){								//Evitar mudar o valor na primeira interação.
			soma = 0;
			somaderivada = 0;
			x0 = x1;
		}
		...
		x1 = x0 - (soma/somaderivada);					//Calcula a solução
		interacoes++;
	}
